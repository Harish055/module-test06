...
let searchInput=document.getElementById("searchbar1");
let searchbtn=document.getElementById("searchbtn");
let studentinfo=document.getElementById("studentdata");
let azbtn=document.getElementById("sortaz");
let zabtn=document.getElementById("sortza");
let marksbtn=document.getElementById("sortmarks");
let passingbtn=document.getElementById("sortpassing");
let classbtn=document.getElementById("sortclass");
let genderbtn=document.getElementById("sortgender");

azbtn.addEventListener("click", sortazbtn);
zabtn.addEventListener("click", sortzabtn);
marksbtn.addEventListener("click",sortmarksbtn);
classbtn.addEventListener("click",sortclassbtn);
passingbtn.addEventListener("click",showpassing);
...
function loadmockdata(data) {
    ...
    data.map((item) =>{
        let TItems=document.createElement("tr");
        TItems.innerHTML=`
                <td>${item.id}</td>
                <td><img src=${item.img_src} alt="" width="50" height="60">${item.first_name} ${item.last_name}</td>
                <td>${item.gender}</td>
                <td>${item.class}</td>
                <td>${item.marks}</td>
...
function sortazbtn () {
    let sortazdata=data.sort((a, b) => {
        if (a.first_name<b.first_name) return -1;
        else if (a.first_name>b.first_name) return 1;
        else return 0;
    })
    loadmockdata(sortazdata);
}

function sortzabtn () {
    let sortzadata=data.sort((a, b) => {
        if (a.first_name<b.first_name) return 1;
        else if (a.first_name>b.first_name) return -1;
        else return 0;
    })
    loadmockdata(sortzadata);
}

function sortmarksbtn () {
    let sortmarksdata=data.sort((a, b) => {
        if (a.marks<b.marks) return -1;
        else if (a.marks>b.marks) return 1;
        else return 0;
    })
    loadmockdata(sortmarksdata);
}
function sortclassbtn () {
    let sortclassdata=data.sort((a, b) => {
        if (a.class<b.class) return -1;
        else if (a.class>b.class) return 1;
        else return 0;
    })
    loadmockdata(sortclassdata);
}
function showpassing () {
    let passingdata=data.filter((item)=>{
        return item.passing;
    })
    loadmockdata(passingdata);
}
function search () {
